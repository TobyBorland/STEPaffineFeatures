...
filepath = testDir + "/RR_STEP_test_N.step"
...
# Reposition curve on both sides of axis in plane of axis and local centroid to find maxima/minima
...
def ellipseParse(localCentroid, ParsedEdge, STEP_entities_):
    ...
    cleanEdgeParams = cleanSubRefs(edgeParams_)
    majorRadius = cleanEdgeParams[-2]
    ParsedEdge['majorRadius'] = majorRadius

    minorRadius = cleanEdgeParams[-1]
...
def BoundedSurface(localCentroid, complexEntityList, STEP_entities_, minimaExtrema=False):
    ...
    typeNameList = [cel.type_name for cel in STEP_entities[complexEntityList].params]
    ...
    if typeNameList[1] == 'B_SPLINE_SURFACE':
        ...
        BsplineSurfaceRefs = STEP_entities[complexEntityList].params[1].params
    ...
    if typeNameList[2] == 'B_SPLINE_SURFACE_WITH_KNOTS':
        BsplineSurfaceWithKnotsRefs = STEP_entities[complexEntityList].params[2].params
    ...
    if typeNameList[4] != 'RATIONAL_B_SPLINE_SURFACE':
        ParsedSurface['weightSpec'] = STEP_entities[complexEntityList].params[4].params[0]

        weights = [item for sublist in ParsedSurface['weightSpec'][0] for item in sublist]
    ...
    if (len(STEPknotUvector) - len(controlPoints) - ParsedSurface['surfaceUdegree']) != 1:
    ...
    if (len(STEPknotVvector) - len(controlPoints) - ParsedSurface['surfaceVdegree']) != 1:
    ...
    ctrlptsw = compatibility.combine_ctrlpts_weights(controlPoints, weights=weights)
...
def edgeParse(c, ParsedEdge, STEP_entities_):
    ...
    if (ParsedEdge['typeName'] == 'TRIMMED_CURVE'): # identified here?
        _1=1
...
def BSplineSurfaceWithKnotsParse(AFS, localCentroid, STEP_entities_, minimaExtrema=False):
    ...
    if AFS.get('ParsedSurface') is not None:
        ...
        ParsedSurface['closedU'] = 'T' in closedU
        ParsedSurface['closedV'] = 'T' in closedV
        ParsedSurface['selfIntersect'] = 'T' in selfIntersect
